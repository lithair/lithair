...
use helpers::{TestServer, LoadTester, PersistenceChecker, CliTester};
...
mod startup {
    ...
    async fn test_server_starts_with_default_config() {
        ...
        let mut server = TestServer::start_default(19100)
            .await
...
"#;
        ...
        let mut server = TestServer::start(19101, config)
            .await
            .expect("Failed to start server");
...
mod api_tests {
    ...
    async fn test_api_get_endpoint() {
        ...
        let mut server = TestServer::start_default(19102)
            .await
            .expect("Failed to start server");
    ...
    async fn test_api_post_endpoint() {
        ...
        let mut server = TestServer::start_default(19103)
            .await
            .expect("Failed to start server");
        ...
        let response = server.post("/api/articles", data)
            .await
...
mod performance_tests {
    ...
    async fn test_server_handles_concurrent_requests() {
        ...
        let mut server = TestServer::start_default(19104)
            .await
            .expect("Failed to start server");
        ...
        let result = tester.run_concurrent_requests(&url, 50)
            .await
            .expect("Load test failed");
        
        println!("📊 Résultats : {}/{} réussies ({:.1}%)", 
                 result.success, result.total, result.success_rate());
...
"#, persistence_path);
        ...
        let mut server = TestServer::start(19105, &config)
            .await
            .expect("Failed to start server");
        
        // Faire quelques requêtes pour générer des events
        server.post("/api/test", serde_json::json!({"data": "test"}))
            .await
            .ok();
...
mod cli_tests {
    ...
    fn test_help_command() {
        ...
        assert!(help.contains("Lithair") || help.contains("Usage"),
