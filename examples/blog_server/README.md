...
cargo run --example blog_server

# Or with custom port
cargo run --example blog_server -- --port 3001
```
...
```bash
mkdir -p examples/blog_server/frontend
echo '<h1>My Blog</h1>' > examples/blog_server/frontend/index.html
```
...
- See `examples/rbac_session_demo` for more RBAC patterns
