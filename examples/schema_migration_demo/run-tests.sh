...
BASELINE_DIR="examples/schema_migration_demo/baseline"
...
# Build the demo
echo -e "${YELLOW}[3/5]${NC} Building schema_migration_demo..."
cargo build -p schema_migration_demo --quiet
echo -e "  ${GREEN}✓${NC} Build complete"

# Start server in background
echo -e "${YELLOW}[4/5]${NC} Starting server (mode: $MODE, port: $PORT)..."
cargo run -q -p schema_migration_demo --bin schema_demo -- -p $PORT -m $MODE 2>&1 &
SERVER_PID=$!
...
echo ""
cargo run -q -p schema_migration_demo --bin schema_demo -- -p $PORT --test
