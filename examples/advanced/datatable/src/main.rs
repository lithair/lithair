...
// ============================================================================
// COLUMN META (for sidebar filters)
// ============================================================================
...
fn compute_column_meta<T: Serialize>(items: &[T], dataset_name: &str) -> MetaResponse {
    if items.is_empty() {
        return MetaResponse { dataset: dataset_name.to_string(), row_count: 0, columns: vec![] };
    }

    let first_val = serde_json::to_value(&items[0]).unwrap_or_default();
    let col_names: Vec<String> = match &first_val {
    ...
    let mut col_strings: HashMap<String, Vec<String>> =
        col_names.iter().map(|n| (n.clone(), Vec::with_capacity(items.len()))).collect();

    for item in items {
        if let Ok(serde_json::Value::Object(map)) = serde_json::to_value(item) {
            for (key, val) in &map {
                if let Some(vals) = col_strings.get_mut(key) {
                    let s = match val {
    ...
    let columns: Vec<ColumnMeta> = col_names
        .iter()
        .map(|name| {
            let vals = col_strings.get(name).unwrap();
            let non_empty: Vec<&str> =
                vals.iter().filter(|v| !v.is_empty()).map(|v| v.as_str()).collect();
            let numeric_count = non_empty.iter().filter(|v| v.parse::<f64>().is_ok()).count();
            let is_numeric =
                !non_empty.is_empty() && (numeric_count as f64 / non_empty.len() as f64) >= 0.9;
            ...
            let mut sorted: Vec<ValueCount> = counts
                .into_iter()
            ...
            sorted.truncate(if is_numeric { 50 } else { 200 });

            ColumnMeta {
                name: name.clone(),
                col_type: if is_numeric { "numeric" } else { "text" }.to_string(),
                distinct_count,
                values: sorted,
                min,
                max,
    ...
    MetaResponse { dataset: dataset_name.to_string(), row_count: items.len(), columns }
}
...
#[derive(Parser)]
#[command(name = "datatable", about = "Lithair DataTable Demo - Multi-Table Relational")]
struct Args {
    #[arg(short, long, default_value = "3001")]
    port: u16,

    #[arg(long, default_value = "examples/advanced/datatable/data")]
    data_dir: PathBuf,
}
...
async fn main() -> Result<()> {
    ...
    println!();
    println!("Column Meta (sidebar filters):");
    println!("  GET /api/products/meta   - Column metadata for products");
    println!("  GET /api/consumers/meta  - Column metadata for consumers");
    println!("  GET /api/orders/meta     - Column metadata for orders");
    println!();
    ...
    match frontend_engine.load_directory("examples/advanced/datatable/frontend").await {
    ...
    LithairServer::new()
        ...
        // ========== META ENDPOINTS (column metadata for sidebar filters) ==========
        .with_route(Method::GET, "/api/products/meta", {
            let handler = product_handler.clone();
            move |_req| {
                let handler = handler.clone();
                Box::pin(async move {
                    let items = handler.get_all_items().await;
                    let meta = compute_column_meta(&items, "products");
                    json_response(&serde_json::to_value(&meta).unwrap_or_default())
                })
            }
        })
        .with_route(Method::GET, "/api/consumers/meta", {
            let handler = consumer_handler.clone();
            move |_req| {
                let handler = handler.clone();
                Box::pin(async move {
                    let items = handler.get_all_items().await;
                    let meta = compute_column_meta(&items, "consumers");
                    json_response(&serde_json::to_value(&meta).unwrap_or_default())
                })
            }
        })
        .with_route(Method::GET, "/api/orders/meta", {
            let handler = order_handler.clone();
            move |_req| {
                let handler = handler.clone();
                Box::pin(async move {
                    let items = handler.get_all_items().await;
                    let meta = compute_column_meta(&items, "orders");
                    json_response(&serde_json::to_value(&meta).unwrap_or_default())
                })
            }
        })
