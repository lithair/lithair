...
```bash
cargo run -p raft_replication_demo --bin http_hardening_node -- --port 18320 --open
```

Vous pouvez aussi compiler l’exemple en mode « ouvert par défaut » via la feature :

```bash
cargo run -p raft_replication_demo --features open_by_default --bin http_hardening_node -- --port 18320
```
...
```
EXPERIMENT_DATA_BASE=examples/raft_replication_demo/data
```
