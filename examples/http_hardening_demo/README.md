...
- Per‑request timeout (RS_HTTP_TIMEOUT_MS)
...
- 413 Payload Too Large on oversized bulk request (limit: RS_HTTP_MAX_BODY_BYTES_BULK)
- 405 Method Not Allowed with Allow header on wrong verb
- 504 Gateway Timeout (best‑effort) when RS_HTTP_TIMEOUT_MS is set very low

Notes:
- Timeouts depend on scheduling; setting `RS_HTTP_TIMEOUT_MS=1` ms typically yields 504 on commodity hosts, but is not guaranteed on very fast responses. Consider raising to a tiny value like 5–10ms if needed.
- You can tune size limits via `RS_HTTP_MAX_BODY_BYTES_SINGLE`/`RS_HTTP_MAX_BODY_BYTES_BULK` to make 413 more or less strict.
