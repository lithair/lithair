...
- Hardening, Gzip, Route Policies: see `./http_hardening_gzip_firewall.md`
...
cargo run -p replication --bin replication-hardening-node -- --port 18320
...
`examples/09-replication/http_loadgen_demo.rs`
(`replication-loadgen`) supports modes:
...
`examples/09-replication/bench_http_server_stateless.sh` orchestrates runs and
writes Markdown reports under `baseline_results/`.
