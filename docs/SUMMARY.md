# Summary

[Introduction](README.md)

---

# Getting Started

- [Installation](guides/getting-started.md)
- [Data-First Philosophy](guides/data-first-philosophy.md)

---

# Concepts

- [DeclarativeModel](features/declarative/overview.md)
- [Attributes](reference/declarative-attributes.md)
  - [Database](modules/storage/database.md)
  - [HTTP](modules/http-server/README.md)
  - [Permissions](modules/rbac/declarative-patterns.md)
  - [Persistence](modules/storage/event-sourcing.md)

---

# Features

- [Live updates over SSE](features/sse.md)
- [Host-header routing](features/vhost.md)
- [Sessions and authentication](features/sessions.md)
- [Memory/disk retention](features/retention.md)

---

# Tutorials

- [E-commerce](guides/ecommerce-tutorial.md)
- [RBAC](guides/rbac.md)
- [Clustering](features/clustering/overview.md)

---

# Architecture

- [Overview](architecture/overview.md)
- [Data Flow](architecture/data-flow.md)
- [System Design](architecture/system-design.md)

---

# Reference

- [Configuration](configuration-reference.md)
- [Environment Variables](reference/env-vars.md)
- [API Reference](reference/api-reference.md)
