...
async fn given_scc2_enabled(world: &mut LithairWorld) {
...
async fn concurrent_connections(world: &mut LithairWorld, client_count: u32) {
    ...
    for i in 0..client_count {
        let world_clone = unsafe { std::mem::transmute::<&mut LithairWorld, &mut LithairWorld>(world) };
        
        let task = tokio::spawn(async move {
            let response = world_clone.make_request("GET", &format!("/perf/echo?client={}", i), None).await;
            response.is_ok()
        });
    ...
    let mut metrics = world.metrics.lock().await;
    metrics.error_rate = ((client_count - success_count as u32) as f64 / client_count as f64) * 100.0;
}
