...
{
    pub fn new(event_store: Arc<std::sync::RwLock<EventStore>>, config: Scc2EngineConfig) -> Result<Self, crate::Error> {
        ...
        F: FnOnce(&S) -> R,
    {
        ...
        if let Some(scc::hash_map::Entry::Occupied(o)) = (*self.state_map).try_entry(key.to_string()) {
            Some(f(&o.get().data))
        } else {
            None
        }
    ...
    pub async fn apply_event<E>(&self, key: String, event: E, persist: bool) -> Result<(), crate::Error>
    where
    ...
    pub fn snapshot(&self) -> Result<(), crate::Error> {
        ...
        let json = serde_json::to_string(&snapshot_map).map_err(|e| crate::Error::SerializationError(e.to_string()))?;
    ...
    where E: Event<State = S>
    {
        let mut state_clone = if let Some(current) = self.read(key, |s| s.clone()) {
            current
        } else {
            S::default()
        };
                 ...
                 if policy.indexed || policy.unique {
                     ...
                     if old_val != new_val {
                         if let Some(v) = old_val {
                             let v_str = match v { serde_json::Value::String(s) => s, _ => v.to_string() };
    ...
    fn add_to_index_value(&self, field_name: &str, value: &str, key: &str) {
        if !(*self.indexes).contains_sync(field_name) {
            let _ = (*self.indexes).insert_sync(field_name.to_string(), SecondaryIndex::new(field_name.to_string()));
        }

        // Use try_entry for read access
        if let Some(scc::hash_map::Entry::Occupied(idx_entry)) = (*self.indexes).try_entry(field_name.to_string()) {
    ...
    fn remove_from_index_value(&self, field_name: &str, value: &str, key: &str) {
        if let Some(scc::hash_map::Entry::Occupied(idx_entry)) = (*self.indexes).try_entry(field_name.to_string()) {
             let idx = idx_entry.get();
             if let Some(scc::hash_map::Entry::Occupied(mut o)) = idx.index_map.try_entry(value.to_string()) {
                 let list = o.get_mut();
    ...
    pub fn get_indexed_values(&self, field_name: &str, value: &str) -> Vec<String> {
        if let Some(scc::hash_map::Entry::Occupied(idx_entry)) = (*self.indexes).try_entry(field_name.to_string()) {
             let idx = idx_entry.get();
             if let Some(scc::hash_map::Entry::Occupied(v_entry)) = idx.index_map.try_entry(value.to_string()) {
    ...
    S: Clone + Send + Sync + Default + Serialize + 'static + Inspectable + ModelSpec,
{
    fn fetch_by_id(&self, id: &str) -> Option<serde_json::Value> {
        self.read(id, |state| serde_json::to_value(state).ok())
            .flatten()
    }
}
