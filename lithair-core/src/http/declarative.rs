...
pub trait HttpExposable: Serialize + DeserializeOwned + Clone + Send + Sync + 'static {
    ...
    /// Optional storage adapter factory generated by `#[storage(...)]`.
    /// `None` keeps the native memory-first/event-sourced handler. Adapters are
    /// dependencies of the application, so core has no SQL-driver dependency.
    fn storage_factory() -> Option<crate::app::ModelFactory> {
        None
    }
    ...
    T: HttpExposable + LifecycleAware + ReplicatedModel + RetentionAware,
{
    ...
    pub fn new(event_store_path: &str) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        if T::storage_factory().is_some() {
            return Err("This model declares external storage; register it with with_model instead of constructing a native handler".into());
        }
