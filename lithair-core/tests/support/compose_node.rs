...
async fn prepare() -> anyhow::Result<()> {
    ...
    for id in 1..=3 {
        ...
        local.keys.retain(|key, _| *key == id);
        local.certs.retain(|key, _| *key <= 3);
...
async fn serve(id: u64, credentials: Credentials) -> anyhow::Result<()> {
        ...
        credentials.keys.len() == 1 && credentials.keys.contains_key(&id),
        "node must receive only its own private key"
    );
    let mut addresses = BTreeMap::new();
    for peer in 1..=3 {
        addresses.insert(peer, address(&format!("raft{peer}"), 9443).await?);
    }
    let network = credentials.transport(id, "compose-qualification", credentials.peers(&addresses));
    let listener = TcpListener::bind(addresses[&id]).await?;
    let control = TcpListener::bind(address(&format!("control{id}"), 8080).await?).await?;
    let log = DurableLog::<TypeConfig>::open_for_node("/data", network.node_identity()?).await?;
